...
if(isset($_GET['id'])) {
    ...
    $stmt = $db->prepare("SELECT PaperFile FROM  publishedpapers  WHERE id = :customer_id");
    ...
    if ($row = $stmt->fetch()) {
        ...
        if (!empty($cvData)) {
            ...
            header('Content-Disposition: attachment; filename="Paper.pdf"'); // Change the filename as needed
