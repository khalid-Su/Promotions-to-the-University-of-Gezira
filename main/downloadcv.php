...
if(isset($_GET['id'])) {
    ...
    $stmt = $db->prepare("SELECT CV FROM customer WHERE customer_id = :customer_id");
    ...
    if ($row = $stmt->fetch()) {
        ...
        if (!empty($cvData)) {
            ...
            header('Content-Disposition: attachment; filename="cv.pdf"'); // Change the filename as needed
